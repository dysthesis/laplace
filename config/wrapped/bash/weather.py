...
def print_dwl(args, forecast):
    if args.debug:
        print("[-] Converting observation data to dwl output")

    print(f"{forecast['icon']} {forecast['temperature']}°C {forecast['precis']}")
...
def main():
    ...
    if not args.output:
        print("Error: An output type is required, options: (terminal, dwl)")
