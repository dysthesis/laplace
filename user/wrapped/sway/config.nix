...
let
  ...
  monitorToSwayConfig =
    ...
    let
      fragments = [
        ...
        (lib.optionalString (
          monitor.pos.x != null && monitor.pos.y != null
        ) "pos ${toString monitor.pos.x} ${toString monitor.pos.y}")
  ...
  scratchpadCfg = scratchpads |> lib.map mkScratchpad |> lib.concatStringsSep "\n";
