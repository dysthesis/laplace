...
let
  ...
  renderArgs =
    ...
    if args == [ ] then
      ...
      " " + concatStringsSep " " (map renderValue args);
  ...
  renderAssignments =
    ...
    in
    if parts == [ ] then "" else " " + concatStringsSep " " parts;
  ...
  renderNode =
    ...
    let
      ...
      commentBlock =
        optionalString (node.comment != null && node.comment != "")
          "${indent}// ${node.comment}\n";
    in
    if node.inline || node.children == [ ] then
      if node.children != [ ] then
        throw "Inline nodes cannot have children (${node.name})"
      else
        "${commentBlock}${header}\n"
    else
      let
  ...
  valueToNodes =
    ...
    if value == null then
      ...
      concatMap (v: valueToNodes name v) value
    else if builtins.isAttrs value then
      let
        spec =
          value
          // {
            name = lib.attrByPath [ "__name" ] (lib.attrByPath [ "name" ] name value);
          };
      in
      [ normalizeNode spec ]
  ...
  normalizeNode =
    ...
    let
      nodeName =
        if spec ? name then
          spec.name
        else if spec ? __name then
          spec.__name
        else
          throw "KDL node is missing a name";
      ...
      childNodes = childrenFromBody ++ map normalizeNode rawChildren;
    in
    {
      name = nodeName;
      inline = inline;
  ...
  normalizeNodes = map normalizeNode;
in
{
  node = attrs: attrs;
  statement =
    { name, args ? [ ], assign ? { }, comment ? null }:
    {
      inherit name args comment assign;
      inline = true;
    };
  block =
    { name, args ? [ ], assign ? { }, body ? { }, children ? [ ], comment ? null }:
    {
      inherit name args comment assign;
      inline = false;
      __body = body;
  ...
  render =
    ...
    nodes
    |> normalizeNodes
    |> concatMapStringsSep "" (renderNode 0);
  inherit renderNode;
}
