  ...
  normaliseNode = spec:
    ...
    then
      let
        ...
        _ = builtins.trace "children of ${nodeName}: ${lib.concatStringsSep ", " (map builtins.typeOf childNodes)}" null;
      in {
        inherit inline args assignments comment;
        name = nodeName;
        children = childNodes;
      }
    else
      throw "KDL node spec must be an attrset, got ${builtins.toString spec}";
...
in {
  ...
  render = nodes:
    let
      _ = builtins.trace "node types: ${lib.concatStringsSep ", " (map builtins.typeOf nodes)}" null;
    in
      nodes
      |> normaliseNodes
