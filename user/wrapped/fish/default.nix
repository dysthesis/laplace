  ...
  in
    {
      ...
      cat = "${getExe bat}";
      notes = "${
        getExe inputs.daedalus.packages.${system}.default
      } new-session -As Notes -c ~/Documents/Notes/Contents ${getExe direnv} exec . $EDITOR";
      temp = "cd $(mktemp -d)";
  ...
  config =
    ...
    ''
      ...
      if status is-interactive
        ...
        ${lib.getExe pkgs.task} next due.before:today+7d
      end
